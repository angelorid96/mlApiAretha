...
const aretha = (q) => ({
	...
	innerHTML_childs: (c) => {//obtiene contenido html de los hijos de un elemento; retorna un array
		childs = [];
		el = document.querySelector(q).querySelectorAll(c)
		// console.log(el);
		if (el.length > 1) {
			for (var item of el) {
				childs.push(item.innerHTML);
			}
	...
	get: (json) => {
		...
		if (Object.keys(json.data).length != 0) {
			// console.log('debug:'+json.url+('?'+json.data));
			xhr.open('GET', json.url + ('?' + json.data), true);
		} else {
			xhr.open('GET', json.url, true);
		}
		// xhr.setRequestHeader('Authorization','Bearer LCICubheWJBZIZLrLeZZgCxDKsfp3d7k');
		xhr.send();
	},
...
function afLoadPage(e, u, v, p) {
	aretha(e).html(afloader);
	aretha().get({
		"url": u,
		"data": {},
...
window.addEventListener('load', async () => {
	...
	// 	// intiSW=require('arethafw/plugins/workbox/js/init.sw.js');
	// 	aretha().addScript('arethafw/plugins/workbox/js/init.sw.js','module');
